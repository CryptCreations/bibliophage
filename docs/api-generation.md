...
- https://github.com/connectrpc/connect-python/blob/main/docs/usage.md
- https://github.com/connectrpc/connect-python/blob/main/docs/getting-started.md

## How This All Kind of Sort of Works for Python
Pretty similar to the gRPC paragraph above. `protoc` is pretty finicky about directory paths, so we just stick with paths like `api/bibliophage/v1alpha1/pdf.proto`. This maps nicely to python packages a la `bibliophage.v1alpha1.pdf_connect`, because protoc treats evey subdirectory under `api/` as a package. Until we really want some other structure, just leave it as it is. Each time i mess with the directories, it ends up as a 30 minute fiddling session, so consider whether this is worth your time.



## How This all Kind of Sort of Works for Typescript (Connect)
